...
public class ForwardCommand implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        if (args.isEmpty()) {
            channel.sendMessage("Please provide a duration to forward (in seconds)").queue();
            return;
        }
        ...
        channel.sendMessageFormat("\u23E9 Fast forwarded **%s** ", String.valueOf(durationSeconds) + " seconds ahead in the track").queue();
    }
    ...
    public String getHelp() {
        return "Fast forwards the current track by an amount in seconds" +
                "\nUsage: `zforward [duration in seconds]`";
    }
}
